  ...
  return (
    <>
      ...
      <nav className="fixed top-0 z-50 w-full border-b border-horizon bg-void-black/90 backdrop-blur-md">
        ...
        {/* Mobile dropdown */}
        {isOpen && (
          <div className="md:hidden border-t border-horizon bg-void-black/95 backdrop-blur-md px-6 py-4 flex flex-col gap-4">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
            ...
            <Link
              href="#rent"
              className="rounded-lg bg-gold px-5 py-2.5 font-display text-sm font-semibold text-void-black hover:bg-gold-light transition-colors duration-200 text-center"
              onClick={() => setIsOpen(false)}
            >
              Rent a Unit
            </Link>
